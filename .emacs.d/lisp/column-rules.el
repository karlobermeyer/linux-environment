;;; column-rules.el --- Graphically indicate the fill column

;; Copyright (c) 2013

;; Author: Karl Obermeyer
;; Version: 0.1
;; Keywords: vertical column rules
...
;; length of the editing window.  column-rules implements this
;; facility in Emacs.
...
;; been fixed in Emacs trunk.  See github.com/alpaker/Column-Rules/issues/31
...
;;
;;   (require 'column-rules)
;;
;; in your init file.

;; To toggle column rules in a buffer, use the
;; command `crs-mode'.
...
;;
;;     github.com/alpaker/Column-Rules
;;
...
(unless (version<= "22" emacs-version)
  (error "column-rules requires version 22 or later"))

;;; ---------------------------------------------------------------------
;;; User Options
;;; ---------------------------------------------------------------------

(defgroup column-rules nil
  "Draw column rules."
  :tag "Column Rules"
  :group 'convenience
  :group 'fill)
...
(provide 'column-rules)

;;; column-rules.el ends here
